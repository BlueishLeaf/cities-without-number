...
export class CitiesWithoutNumberActorSheet extends ActorSheet {
  ...
  getData() {
    ...
    this.actor.items
      .filter((item) => item._sheet && item._sheet.rendered)
  ...
  async _onDropItem(event, data) {
    ...
    if (
      !["drone", "vehicle", "server"].includes(this.actor.type) &&
      [
        "mod",
  ...
  _prepareOperatorData(context) {
    if (
      !this.actor.system.operator ||
      this.actor.system.operator === "undefined"
    )
      return;

    const operatorFound = game.actors.find(
      (actor) => actor._id === this.actor.system.operator,
    );
    if (!operatorFound) return;

    const operator = operatorFound.toObject(false); // Use safe clone
    const operatorSkills = operator.items.filter(
      (item) => item.type === "skill",
    );
    const operatorDriveSkill = operatorSkills.find(
      (skill) => skill.name === "Drive",
    );
  ...
  _setSkillCap(itemCollection, capSkill, sourceSkill) {
    const cappedSkill = itemCollection.find(
      (item) => item.type === "skill" && item.name === sourceSkill.name,
    );
    cappedSkill.system.level =
      capSkill.system.level < sourceSkill.system.level
  ...
  _prepareOperators(context) {
    ...
    game.actors
      ...
      .forEach(
        (character) => (context.operators[character._id] = character.name),
      );
  }
  ...
  _prepareCharacterData(context) {
    ...
    if (context.system.systemStrain.maxModifier) {
      context.system.systemStrain.max += Number(
        context.system.systemStrain.maxModifier,
      );
    }
    if (context.system.lifestyle.systemStrainMod) {
      context.system.systemStrain.max += Number(
        context.system.lifestyle.systemStrainMod,
      );
    }
    context.system.systemStrain.permanent = 0;
    if (context.system.systemStrain.permanentModifier) {
      context.system.systemStrain.permanent += Number(
        context.system.systemStrain.permanentModifier,
      );
    }

    // Set max and permanent alienation score
    console.info(context.system);
    context.system.alienationScore.max =
      context.system.abilities.wis.value +
      context.system.alienationScore.maxModifier;
    context.system.alienationScore.permanent =
      context.system.alienationScore.permanentModifier;

    // Set stowed and readied items
    context.system.encumbrance.stowed.max =
      context.system.abilities.str.value +
      context.system.encumbrance.stowed.maxBonus;
    context.system.encumbrance.readied.max =
      Math.floor(context.system.abilities.str.value / 2) +
      context.system.encumbrance.readied.maxBonus;
    ...
    context.items.forEach((item) => {
      if (item.system.readied !== undefined) {
        if (item.system.readied && !item.system.wearable) {
          context.system.encumbrance.readied.value +=
            item.system.encumbrance * item.system.quantity;
        } else {
          context.system.encumbrance.stowed.value +=
            item.system.encumbrance * item.system.quantity;
        }
      }
      ...
      if (item.type === "cyberware" && item.system.systemStrain) {
        ...
        if (item.system.chromeSyndromes) {
          item.system.chromeSyndromes.forEach((id) => {
            ...
            if (syndrome) {
              context.system.alienationScore.max +=
                syndrome.system.maxAlienationMod;
    ...
    context.system.systemStrain.value =
      context.system.systemStrain.permanent +
      context.system.systemStrain.temporary;
    context.system.alienationScore.value =
      context.system.alienationScore.permanent +
      context.system.alienationScore.temporary;

    // Calculate maintenance score
    const fixSkill = context.skill.find(
      (skill) => skill.name.toLowerCase() === "fix",
    );
    if (fixSkill) {
      // Floor the value to 0 if fix is -1 or something
      context.system.maintenanceScore.max = Math.max(
        context.system.abilities["int"].mod +
          context.system.abilities["con"].mod +
          3 * fixSkill.system.level +
          context.system.maintenanceScore.maxBonus,
        0,
      );
    }
  }
  ...
  _prepareDroneData(context) {
    ...
    context.items.forEach((item) => {
      if (CONFIG.CWN.inventoryItemTypes.includes(item.type)) {
        context.system.cargoEncumbrance.stowed.value +=
          item.system.encumbrance * item.system.quantity;
  ...
  _prepareCyber(context) {
    ...
    this.actor.system.cyberwareMaintenanceCost =
      context.cyberwareMaintenanceCost;
  }

  _prepareBiographyData(context) {
    context.goals = context.system.goals;
    context.languages = context.system.languages;
    context.totalMonthlyCost =
      context.cyberwareMaintenanceCost +
      context.system.lifestyle.monthlyCost +
      context.system.miscMonthlyCosts;
  }
  ...
  async _onListItemCreate(event) {
    ...
    if (this.actor.system[listType] instanceof Array) {
      ...
      ]).then((updates) => console.log("Updated actor", updates));
    }
  }
  ...
  async _onListItemDelete(event) {
    ...
    if (this.actor.system[listType] instanceof Array) {
      const idx = this.actor.system[listType].findIndex(
        (item) => item === event.currentTarget.parentNode.children[0].value,
      );
      if (idx > -1) {
  ...
  updateReadiedFlag(event) {
    ...
    let itemUpdates = [
      { _id: selectedItem._id, system: { readied: newReadiedState } },
    ];
    // Un-equip other armor if readied item is also armor
    if (
      selectedItem.type === "armor" &&
      selectedItem.system.subType === "armor" &&
      selectedItem.system.readied
    ) {
      const otherEquippedArmorItems = this.actor.items.filter(
        (actorItem) =>
          actorItem.type === "armor" &&
          actorItem.system.subType === "armor" &&
          actorItem.system.readied &&
          actorItem.id !== selectedItem.id,
      );
      otherEquippedArmorItems.forEach((equippedArmorItem) =>
        itemUpdates.push({
          _id: equippedArmorItem._id,
          system: { readied: false },
        }),
      );
      ...
      if (selectedItem.system.isSuit) {
        const incompatibleAccessories = this.actor.items.filter(
          (actorItem) =>
            actorItem.type === "armor" &&
            actorItem.system.subType === "accessory" &&
            !actorItem.system.canEquipWithSuit &&
            actorItem.system.readied,
        );
        incompatibleAccessories.forEach((incompatibleAccessory) =>
          itemUpdates.push({
            _id: incompatibleAccessory._id,
            system: { readied: false },
          }),
        );
      }
    }
    Item.updateDocuments(itemUpdates, { parent: this.actor }).then(
      (updatedItems) => console.log("Updated items", updatedItems),
    );
  }
  ...
  updateSkillLevel(event) {
    ...
    Item.updateDocuments(
      [
        {
          _id: skill._id,
          system: { level: newSkillLevel },
        },
      ],
      { parent: this.actor },
    ).then((updatedSkill) => console.log("Updated skill", updatedSkill));
  }
  ...
  openCreateItemDialog(itemData) {
    ...
    const createItemDialog = new Dialog({
      ...
      buttons: DialogUtils.createButtons((html) =>
        this.handleCreateCustomItem(itemData, html),
      ),
      default: "create",
    });
  ...
  _onConfig(event) {
    ...
    const configDialog = new Dialog({
      ...
      buttons: DialogUtils.confirmButtons((html) =>
        actorConfigHandlers[dataset.configType](this.actor, html),
      ),
      default: "roll",
    });
  ...
  openBasicSkillDialog(skillBonus) {
    const skillDialog = new Dialog({
      ...
      buttons: DialogUtils.rollButtons((html) =>
        this.handleBasicSkillRoll(skillBonus, html),
      ),
      default: "roll",
    });
  ...
  rollBasicSkill(rollData) {
    ...
    roll
      .toMessage({ speaker, rollMode, flavor })
  ...
  openMoraleDialog(moraleTarget) {
    const moraleDialog = new Dialog({
      ...
      content: DialogTemplates.basicRollDialog(),
      buttons: DialogUtils.rollButtons((html) =>
        this.handleMoraleRoll(moraleTarget, html),
      ),
      default: "roll",
    });
  ...
  rollMorale(moraleCheck, rollData) {
    ...
    roll.render().then((rollRender) => {
      ...
      const content = ChatRenders.passOrFailRender(
        rollRender,
        moraleCheckPassed,
      );
      ChatMessage.create({ ...messageData, content }).then((message) =>
        console.log(message),
      );
    });
  }
  ...
  openAlienationDialog(alienationScoreValue) {
    const alienationDialog = new Dialog({
      ...
      content: DialogTemplates.basicRollDialog(),
      buttons: DialogUtils.rollButtons((html) =>
        this.handleAlienationRoll(alienationScoreValue, html),
      ),
      default: "roll",
    });
  ...
  rollAlienation(alienationCheck, rollData) {
    ...
    roll.render().then((rollRender) => {
      ...
      const messageData = ChatUtils.initializeChatData(
        this.actor,
        alienationCheck,
      );
  ...
  handleGunnerSelect(weapon) {
    ...
    const gunnerOptions = availableGunners.map(
      (gunner) => `<option value="${gunner._id}">${gunner.name}</option>\n`,
    );
    const vehicleGunnerDialog = new Dialog({
      title: "Choose a gunner",
      content: DialogTemplates.gunnerSelectDialog(gunnerOptions),
      buttons: DialogUtils.confirmButtons((html) =>
        this.handleGunnerConfirmation(html, weapon),
      ),
      default: "confirm",
    });
  ...
  handleGunnerConfirmation(html, weapon) {
    ...
    const selectedGunner = game.actors.find(
      (actor) => actor._id === selectedGunnerId,
    );
  ...
  openSkillDialog(skill) {
    ...
    const skillDialog = new Dialog({
      ...
      content: DialogTemplates.skillRollDialog(
        abilityOptions,
        specialistOptions,
      ),
      buttons: DialogUtils.rollButtons((html) =>
        this.handleSkillRoll(skill, html),
      ),
      default: "roll",
    });
    skillDialog.render(true);
  ...
  handleSkillRoll(skill, html) {
    ...
    const selectedSpecialistLevel = html
      .find('[name="specialistSelect"]')
      .val();
      ...
      [
        {
          _id: skill._id,
          system: {
            attribute: selectedAttributeCode,
            specialist: selectedSpecialistLevel,
          },
        },
      ],
      { parent: this.actor },
    ).then((updates) => console.log("Updated skill", updates));

    // Apply armor penalty if applicable to this skill
    let armorPenalty = 0;
    const equippedHeavyArmorItems = this.actor.items.filter(
      (item) =>
        item.type === "armor" && item.system.readied && item.system.isHeavy,
    );
    if (
      ["exert", "sneak"].includes(skill.name.toLowerCase()) &&
      equippedHeavyArmorItems.length > 0
    ) {
  ...
  rollSkill(skill, rollData) {
    ...
    const roll = new Roll(
      this.determineSkillRollFormula(skill.system.specialist),
      rollData,
    );
