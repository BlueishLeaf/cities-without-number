...
export class CitiesWithoutNumberItemSheet extends ItemSheet {
  ...
  activateListeners(html) {
    ...
    html.find(".item-delete").click((ev) => {
      ...
      if (this.item.system.mods && item.type === "mod") {
        ...
        this.item.system.implantComplications &&
        item.type === "implantComplication"
      ) {
        childCollection = this.item.system.implantComplications;
        systemUpdate = { implantComplications: childCollection };
      } else if (
        this.item.system.chromeSyndromes &&
        item.type === "chromeSyndrome"
      ) {
      ...
      Item.updateDocuments(
        ...
        { parent: this.actor },
      ).then((updates) => console.log("Updated item", updates));
    });
  ...
  async _onDropItemCreate(itemData) {
    ...
    if (this.item.system.mods && itemData[0].type === "mod") {
      ...
      this.item.system.implantComplications &&
      itemData[0].type === "implantComplication"
    ) {
      childCollection = this.item.system.implantComplications;
      systemUpdate = {
        implantComplications: this.item.system.implantComplications,
      };
    } else if (
      this.item.system.chromeSyndromes &&
      itemData[0].type === "chromeSyndrome"
    ) {
      childCollection = this.item.system.chromeSyndromes;
  ...
  _onSortItem(event, itemData) {
    ...
    this.actor
      .updateEmbeddedDocuments("Item", updateData)
      .then(() => this.item.sheet.render(this.item));
  }
  ...
  async _onItemCreate(event) {
    ...
    if (this.item.system.mods && createdItem.type === "mod") {
      ...
      this.item.system.implantComplications &&
      createdItem.type === "implantComplication"
    ) {
      childCollection = this.item.system.implantComplications;
      systemUpdate = { implantComplications: childCollection };
    } else if (
      this.item.system.chromeSyndromes &&
      createdItem.type === "chromeSyndrome"
    ) {
      childCollection = this.item.system.chromeSyndromes;
      systemUpdate = { chromeSyndromes: childCollection };
  ...
  deleteCommandLine(event) {
    const idx = this.item.system.commandLines.findIndex((item) =>
      item.includes(event.currentTarget.parentNode.children[0].value),
    );
  ...
  deleteTargetType(event) {
    const idx = this.item.system.targetTypes.findIndex((item) =>
      item.includes(event.currentTarget.parentNode.children[0].value),
    );
