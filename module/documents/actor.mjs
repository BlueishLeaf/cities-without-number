...
export class CitiesWithoutNumberActor extends Actor {
  ...
  prepareArmor(actorData) {
    ...
    const equippedArmor = actorData.items.find(
      (item) =>
        item.type === "armor" &&
        item.system.readied &&
        item.system.subType === "armor",
    );
    const equippedAccessories = actorData.items.filter(
      (item) =>
        item.type === "armor" &&
        item.system.readied &&
        item.system.subType === "accessory",
    );
    ...
    if (equippedArmor && equippedArmor.system.armorClass.melee > baseMeleeAC) {
      actorData.system.armorClass.melee = equippedArmor
        ? equippedArmor.system.armorClass.melee
        : baseMeleeAC;
    }
    ...
    if (
      equippedArmor &&
      equippedArmor.system.armorClass.ranged > baseRangedAC
    ) {
      actorData.system.armorClass.ranged = equippedArmor
        ? equippedArmor.system.armorClass.ranged
        : baseRangedAC;
    }
    ...
    actorData.system.damageSoak.max = equippedArmor
      ? Number(equippedArmor.system.damageSoak) + baseDamageSoak
      : baseDamageSoak;
  ...
  openNpcWeaponDialog(weapon, useRangedBonus) {
    const weaponDialog = new Dialog({
      ...
      content: DialogTemplates.autoWeaponRollDialog(
        weapon.system.isBurstFireable,
        weapon.system.canDealNonLethalDamage,
      ),
      buttons: DialogUtils.rollButtons((html) =>
        this.handleAutoWeaponRoll(
          weapon,
          this.getBaseAttackBonusByActorType(useRangedBonus, false),
          html,
        ),
      ),
      default: "roll",
  ...
  openAutoWeaponDialog(weapon) {
    const weaponDialog = new Dialog({
      ...
      buttons: DialogUtils.rollButtons((html) =>
        this.handleAutoWeaponRoll(
          weapon,
          this.getBaseAttackBonusByActorType(false, true),
          html,
        ),
      ),
      default: "roll",
  ...
  openWeaponDialog(weapon) {
    ...
    const skillOptions = skills.map(
      (skill) =>
        `<option value="${skill._id}" ${weapon.system.skill === skill.name.toLowerCase() ? "selected" : ""}>${skill.name}</option>\n`,
    );
    const weaponDialog = new Dialog({
      title: `Roll ${weapon.name}`,
      content: DialogTemplates.weaponRollDialog(
        weapon.system.isBurstFireable,
        weapon.system.canDealNonLethalDamage,
        abilityOptions,
        skillOptions,
      ...
      buttons: DialogUtils.rollButtons((html) =>
        this.handleWeaponRoll(weapon, html),
      ),
      default: "roll",
    });
    weaponDialog.render(true);
  ...
  handleWeaponRoll(weapon, html) {
    ...
    const selectedSkill = this.items.get(
      html.find('[name="skillSelect"]').val(),
    );
    const skillMod = selectedSkill.system.level;

    const nonLethalElements = html.find('[name="nonLethalInput"]');
    const isNonLethal =
      nonLethalElements.length > 0 ? nonLethalElements[0].checked : false;

    const burstFireElements = html.find('[name="burstFireInput"]');
    const isBurstFire =
      burstFireElements.length > 0 ? burstFireElements[0].checked : false;
    ...
    if (
      !weapon.system.hasMagazine ||
      this.magazineHasEnoughAmmo(weapon.system.magazine, isBurstFire)
    ) {
      this.rollWeapon(weapon, isNonLethal, isBurstFire, {
        attributeMod,
        skillMod,
        baseAB,
        situationalAB,
      });
      const updatedMagazine = this.getUpdatedMagazine(
        isBurstFire,
        weapon.system.magazine,
      );
        ...
        [
          {
            ...
            system: {
              ...
              skill: selectedSkill.name.toLowerCase(),
              magazine: updatedMagazine,
            },
          },
        ],
        { parent: weapon.actor },
      ).then((updates) => console.log("Updated weapon", updates));
    } else {
    ...
    if (
      ...
      Item.updateDocuments(
        [
          {
            _id: weapon._id,
            system: { magazine: updatedMagazine },
          },
        ],
        { parent: this },
      ).then((updates) => console.log("Updated weapon", updates));
  ...
  getBaseAttackBonusByActorType(useRangedBonus, isAutoFire) {
    if (this.type === "drone") {
      return isAutoFire
        ...
        : game.actors.find((actor) => actor._id === this.system.operator).system
            .attackBonus;
    } else if (this.type === "npc") {
      return useRangedBonus
        ? this.system.attackBonus.ranged
        : this.system.attackBonus.melee;
    }
  ...
  rollWeapon(weapon, isNonLethal, isBurstFire, rollData) {
    ...
    if (
      !isNonLethal &&
      weapon.system.canDealTraumaticHits &&
      weapon.system.damageFormula
    ) {
    ...
    if (rollRenderPromises.length > 0) {
      Promise.all(rollRenderPromises).then((rollRenders) => {
        const messageData = ChatUtils.initializeChatData(this, weapon);
        const content = ChatRenders.buildChatContentForAttackRoll(
          weapon,
          isNonLethal,
          damageRoll,
          rollRenders,
